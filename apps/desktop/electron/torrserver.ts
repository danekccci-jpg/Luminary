...
export class TorrServerManager {
  ...
  private async applyNetworkSettings() {
    ...
    try {
      ...
      if (!hasActiveTorrents) settings.PeersListenPort = 43211;
      await this.settingsRequest('set', settings);
      console.log(`[TorrServer] Network settings applied${hasActiveTorrents ? ' (P2P port deferred: active torrents)' : ', PeersListenPort=43211'}`);
    } catch (e: any) {
   ...
   */
  /** Токен архитектуры в именах релизов YouROK/TorrServer: `process.arch` x64 → amd64, ia32 → 386. */
  private archToken(): string {
   ...
   *  где arch — amd64/arm64/386. gst-сборки существуют только для amd64/arm64
   *  (darwin/linux/windows). Имя совпадает и с bundled-файлом в extraResources,
   *  и с именем ассета для runtime-скачивания (см. scripts/fetch-torrserver.js). */
  public getBinaryName(useGst: boolean = false): string {
    ...
    const token = this.archToken();

    if (platform === 'win32') return `${name}-windows-${token}.exe`;
    if (platform === 'darwin') return `${name}-darwin-${token}`;
    return `${name}-linux-${token}`;
  }
  ...
  public async configureServer(ramCacheMB: number = 300, applyPeersPort: boolean = true): Promise<any> {
    ...
    try {
      ...
      const desired: Record<string, unknown> = {
        ...
        ConnectionsLimit: 150,              // высокий предел без перегрузки anacrolix
        //   TorrServer по умолчанию ограничивает соединения 25; 150 оставляет
        //   запас для раздач с 100+ пирами, но не возвращает прежние паники
        //   anacrolix при агрессивном лимите 250.
        ClientsStatLimit: 30,
        ...
        ResponsiveMode: true,                // не ждать завершения куска перед выдачей данных
