...
export const App: React.FC = () => {
  ...
  const [activeStream, setActiveStream]   = useState<{
    ...
    season?: number;
    episode?: number;
    /** Следующая серия (S/E) — кнопка на энд-скрине плеера. */
    nextEpisode?: { season: number; episode: number } | null;
    /** Запустить следующую серию (резолвит раздачу и пересобирает стрим). */
    onPlayNext?: () => void;
    /** Счётчик запусков — меняется при каждом воспроизведении, чтобы PlayerModal
     *  полностью пересоздавался (следующая серия = чистый старт плеера). */
    nonce: number;
    mediaId?: string;
    mediaType?: 'movie' | 'tv';
    year?: string;
    ...
    >
        ...
        <MovieDetailsModal
          ...
          onPlayTorrent={(torrent) => {
            ...
            setActiveStream({
              ...
              // Новый nonce → PlayerModal полностью пересоздаётся (следующая серия
              // стартует с чистого состояния, даже если это та же раздача-сезонник).
              nonce: Date.now(),
            });
            refreshLibrary();
          }}
      ...
      {activeStream && (
        <PlayerModal
          ...
          directQuality={activeStream.directQuality}
      ...
      {isMagnetModalOpen && (
        <MagnetInputModal
          ...
          onPlayMagnet={(magnet, title) => {
            ...
            setActiveStream({ magnet, title, nonce: Date.now() });
          }}
        />
      )}
